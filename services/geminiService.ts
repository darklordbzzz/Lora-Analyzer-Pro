...
export const analyzeLoraFileWithGemini = async (file: LoraFileWithPreview, hash: string) => {
  const ai = new GoogleGenAI({ apiKey: process.env.API_KEY as string });
  const fileName = file.lora.name;
  try {
    const prompt = `
      ...
      Strictly follow the logic in the script to determine all fields.
